package com.warriorssmp.jobs.cooking;

import com.warriorssmp.jobs.WSMPJobsPlugin;
import com.warriorssmp.jobs.common.JobModule;
import com.warriorssmp.jobs.cooking.command.GatherAdminCommand;
import com.warriorssmp.jobs.cooking.command.GatherCommand;
import com.warriorssmp.jobs.cooking.data.DataStore;
import com.warriorssmp.jobs.common.EconomyService;
import com.warriorssmp.jobs.cooking.listener.GatherListener;
import com.warriorssmp.jobs.cooking.menu.MenuManager;
import com.warriorssmp.jobs.cooking.task.GatherConfig;
import com.warriorssmp.jobs.cooking.task.LeaderboardService;
import com.warriorssmp.jobs.cooking.task.LegendaryRequestService;
import com.warriorssmp.jobs.cooking.task.LuckyStrikeService;
import com.warriorssmp.jobs.cooking.task.MasterNpcService;
import com.warriorssmp.jobs.cooking.task.PremiumService;
import com.warriorssmp.jobs.cooking.task.ShopService;
import com.warriorssmp.jobs.cooking.task.TaskService;

public final class CookingPlugin extends JobModule {
    ...
    public CookingPlugin(WSMPJobsPlugin host) {
        super(host, "cooking");
    }
    ...
    public void onEnable() {
        ...
        registerCommand("cookmenu", gatherCommand);
        registerCommand("cooktask", gatherCommand);
        registerCommand("cookleaderboards", gatherCommand);
        registerCommand("cookbuffs", gatherCommand);
        registerCommand("cookshop", gatherCommand);

        GatherAdminCommand adminCommand = new GatherAdminCommand(this);
        registerCommand("cookshopadmin", adminCommand);
        registerCommand("cookmenuadmin", adminCommand);
        registerCommand("cookeditor", adminCommand);
        registerCommand("cookmaster", adminCommand);

        if (getServer().getPluginManager().getPlugin("PlaceholderAPI") != null) {
            new com.warriorssmp.jobs.cooking.CookingPlaceholders(this).register();
    ...
    public void logStartupSummary() {
        ...
        log.info("==================================================");
        log.info("  WSMP-Jobs / Cooking — Config Summary");
        log.info("==================================================");
        ...
        if (gatherConfig.allTiers().isEmpty()) {
            log.warning("  No tiers loaded at all — check the 'tiers:' section in cooking/config.yml!");
        }
