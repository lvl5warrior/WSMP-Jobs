...
import com.warriorssmp.jobs.fishing.command.GatherAdminCommand;
import com.warriorssmp.jobs.fishing.command.GatherCommand;
import com.warriorssmp.jobs.fishing.data.DataStore;
import com.warriorssmp.jobs.common.EconomyService;
import com.warriorssmp.jobs.fishing.listener.GatherListener;
import com.warriorssmp.jobs.fishing.menu.MenuManager;
import com.warriorssmp.jobs.fishing.task.GatherConfig;
import com.warriorssmp.jobs.fishing.task.LeaderboardService;
import com.warriorssmp.jobs.fishing.task.LegendaryRequestService;
import com.warriorssmp.jobs.fishing.task.LuckyStrikeService;
import com.warriorssmp.jobs.fishing.task.MasterNpcService;
import com.warriorssmp.jobs.fishing.task.PremiumService;
import com.warriorssmp.jobs.fishing.task.ShopService;
import com.warriorssmp.jobs.fishing.task.TaskService;

public final class FishingPlugin extends JobModule {
    ...
    public FishingPlugin(WSMPJobsPlugin host) {
        super(host, "fishing");
    }
    ...
    public void onEnable() {
        ...
        registerCommand("fishmenu", gatherCommand);
        registerCommand("fishtask", gatherCommand);
        registerCommand("fishleaderboards", gatherCommand);
        registerCommand("fishbuffs", gatherCommand);
        registerCommand("fishshop", gatherCommand);

        GatherAdminCommand adminCommand = new GatherAdminCommand(this);
        registerCommand("fishshopadmin", adminCommand);
        registerCommand("fishmenuadmin", adminCommand);
        registerCommand("fisheditor", adminCommand);
        registerCommand("fishmaster", adminCommand);

        if (getServer().getPluginManager().getPlugin("PlaceholderAPI") != null) {
            new com.warriorssmp.jobs.fishing.FishingPlaceholders(this).register();
    ...
    public void logStartupSummary() {
        ...
        log.info("==================================================");
        log.info("  WSMP-Jobs / Fishing — Config Summary");
        log.info("==================================================");
        ...
        if (gatherConfig.allTiers().isEmpty()) {
            log.warning("  No tiers loaded at all — check the 'tiers:' section in fishing/config.yml!");
        }
