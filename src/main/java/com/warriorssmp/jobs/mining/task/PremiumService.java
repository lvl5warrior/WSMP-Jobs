 ...
 *   - the player has the "premiummining" permission (e.g. from a rank plugin)
...
public final class PremiumService {
    ...
    public PremiumService(MiningPlugin plugin) {
        this.plugin = plugin;
    ...
    public boolean isPremium(Player player) {
        return player.isOp() || player.hasPermission("premiummining") || granted.contains(player.getUniqueId());
    }
    ...
    public String describeStatus(OfflinePlayer target) {
        ...
        if (target.getPlayer() != null && target.getPlayer().hasPermission("premiummining")) return "§dPermission";
