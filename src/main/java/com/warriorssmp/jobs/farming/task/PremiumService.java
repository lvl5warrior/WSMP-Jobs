 ...
 *   - the player has the "premiumfarming" permission (e.g. from a rank plugin)
...
public final class PremiumService {
    ...
    public PremiumService(FarmingPlugin plugin) {
        this.plugin = plugin;
    ...
    public boolean isPremium(Player player) {
        return player.isOp() || player.hasPermission("premiumfarming") || granted.contains(player.getUniqueId());
    }
    ...
    public String describeStatus(OfflinePlayer target) {
        ...
        if (target.getPlayer() != null && target.getPlayer().hasPermission("premiumfarming")) return "§dPermission";
