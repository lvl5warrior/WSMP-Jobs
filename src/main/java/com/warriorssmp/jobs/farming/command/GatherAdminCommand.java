...
/**
 * Backs /farmshopadmin, /farmmenuadmin, /farmeditor, and /farmmaster.
 * /farmeditor with no arguments opens the in-game Admin Panel GUI (reload,
...
public final class GatherAdminCommand implements CommandExecutor {
    ...
    public GatherAdminCommand(FarmingPlugin plugin) {
        this.plugin = plugin;
    }

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("farming.admin")) {
            sender.sendMessage("§cYou don't have permission for that.");
            return true;
        }

        switch (command.getName().toLowerCase()) {
            case "farmmaster" -> handleMaster(sender, args);
            case "farmmenuadmin" -> handleMenuAdmin(sender, args);
            case "farmeditor" -> handleEditor(sender, args);
            case "farmshopadmin" -> handleShopAdmin(sender, args);
            default -> {return false;}
        }
        return true;
    }

    // /farmmaster set|remove
    private void handleMaster(CommandSender sender, String[] args) {
        ...
        if (args.length == 0) {
            sender.sendMessage("§eUsage: /farmmaster <set|remove>");
            return;
        }
        if (args[0].equalsIgnoreCase("set")) {
            ...
            villager.setCustomName("§6§lMaster Farmer");
            villager.setCustomNameVisible(true);
            villager.getPersistentDataContainer().set(
                    new NamespacedKey(plugin.host(), "master_farmer_npc"), PersistentDataType.BOOLEAN, true);
            plugin.masterNpcService().setMasterLocation(loc);
            sender.sendMessage("§aMaster Farmer NPC placed. Right-click it to open the Master Farmer menu.");
        } else if (args[0].equalsIgnoreCase("remove")) {
            int removed = 0;
            NamespacedKey key = new NamespacedKey(plugin.host(), "master_farmer_npc");
            ...
            sender.sendMessage("§aRemoved " + removed + " Master Farmer NPC(s) in this world.");
        } else {
            sender.sendMessage("§eUsage: /farmmaster <set|remove>");
        }
    }

    // /farmmenuadmin view <player>
    // /farmmenuadmin setlevel <player> <level>
    // /farmmenuadmin settask <player> <material> <amount>
    private void handleMenuAdmin(CommandSender sender, String[] args) {
        if (args.length == 0) {
            sender.sendMessage("§eUsage: /farmmenuadmin <view|setlevel|settask|setpoints|reset> <player> [args]");
            return;
        }
        switch (args[0].toLowerCase()) {
            case "view" -> {
                if (args.length < 2) {
                    sender.sendMessage("§eUsage: /farmmenuadmin view <player>");
                    return;
                }
                ...
                sender.sendMessage("§6--- Farmer data: " + target.getName() + " ---");
            ...
            case "setlevel" -> {
                if (args.length < 3) {
                    sender.sendMessage("§eUsage: /farmmenuadmin setlevel <player> <level>");
                    return;
                }
                Player target = Bukkit.getPlayerExact(args[1]);
                if (target == null) {
                ...
                sender.sendMessage("§aSet " + target.getName() + "'s Farmer level to " + level + ".");
            }
            case "settask" -> {
                if (args.length < 4) {
                    sender.sendMessage("§eUsage: /farmmenuadmin settask <player> <material> <amount>");
                    return;
                }
                Player target = Bukkit.getPlayerExact(args[1]);
            ...
            case "setpoints" -> {
                if (args.length < 3) {
                    sender.sendMessage("§eUsage: /farmmenuadmin setpoints <player> <amount>");
                    return;
                }
                Player target = Bukkit.getPlayerExact(args[1]);
                if (target == null) {
            ...
            case "reset" -> {
                if (args.length < 2) {
                    sender.sendMessage("§eUsage: /farmmenuadmin reset <player>");
                    return;
                }
                Player target = Bukkit.getPlayerExact(args[1]);
                if (target == null) {
                ...
                PlayerGatherData data = plugin.dataStore().get(target.getUniqueId());
                data.resetAll();
                sender.sendMessage("§cReset all Farming progress for " + target.getName() + ".");
            }
            default -> sender.sendMessage("§eUsage: /farmmenuadmin <view|setlevel|settask|setpoints|reset> <player> [args]");
        }
    }

    // /farmeditor  (no args)          -> opens the in-game Admin Panel GUI
    // /farmeditor reload
    // /farmeditor giveglobalbuff <xp|pointboost|bettertasks> <minutes>
    private void handleEditor(CommandSender sender, String[] args) {
        if (args.length == 0) {
            if (sender instanceof Player player) {
                plugin.menuManager().openAdminPanel(player);
            } else {
                sender.sendMessage("§eUsage: /farmeditor <reload|giveglobalbuff>");
            }
            return;
        }
        switch (args[0].toLowerCase()) {
            case "reload" -> {
                ...
                sender.sendMessage("§aFarmer config reloaded — check console for a full summary.");
            }
            case "giveglobalbuff" -> {
                if (args.length < 3) {
                    sender.sendMessage("§eUsage: /farmeditor giveglobalbuff <xp|pointboost|bettertasks> <minutes>");
                    return;
                }
                long minutes;
                try {
            ...
            default -> sender.sendMessage("§eUsage: /farmeditor <reload|giveglobalbuff>");
        }
    }
    ...
    private void handleShopAdmin(CommandSender sender, String[] args) {
        if (args.length > 0 && args[0].equalsIgnoreCase("list")) {
            sender.sendMessage("§6--- Farmer Shop Items ---");
            plugin.gatherConfig().shopItems().forEach((id, item) ->
                    sender.sendMessage("§7" + id + " §f- " + item.display() + " §7(" + item.cost() + " points"
                            + (item.premium() ? ", premium" : "") + ")"));
            sender.sendMessage("§7Edit prices/items directly in config.yml, then run /farmeditor reload.");
        } else {
            sender.sendMessage("§eUsage: /farmshopadmin list");
            sender.sendMessage("§7Shop items are defined in config.yml — edit there, then /farmeditor reload.");
        }
    }
}
