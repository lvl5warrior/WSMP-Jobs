...
import com.warriorssmp.jobs.common.AbstractDataStore;
...
public final class DataStore extends AbstractDataStore<PlayerGatherData> {

    private final HunterPlugin plugin;

    public DataStore(HunterPlugin plugin) {
        super(plugin, "playerdata");
        this.plugin = plugin;
    }

    @Override
    protected PlayerGatherData load(UUID uuid) {
        ...
        data.lifetimeBossKills = yml.getInt("lifetime-boss-kills", 0);

        for (String typeName : yml.getStringList("blocked-monsters")) {
            try {
                data.blockedMonsters.add(EntityType.valueOf(typeName));
        ...
        if (taskMat != null) {
            try {
                EntityType type = EntityType.valueOf(taskMat);
    ...
    public void save(PlayerGatherData data) {
        ...
        yml.set("lifetime-boss-kills", data.lifetimeBossKills);

        List<String> blocked = new ArrayList<>();
        for (EntityType t : data.blockedMonsters) blocked.add(t.name());
        yml.set("blocked-monsters", blocked);
        ...
        try {
            ...
            plugin.getLogger().warning("Failed to save Combat Master data for " + data.uuid + ": " + e.getMessage());
        }
    }
}
