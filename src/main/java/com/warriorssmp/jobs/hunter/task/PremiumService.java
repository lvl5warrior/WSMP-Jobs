 ...
 *   - the player has the "premiumhunter" permission (e.g. from a rank plugin)
...
public final class PremiumService {
    ...
    public PremiumService(HunterPlugin plugin) {
        this.plugin = plugin;
    ...
    public boolean isPremium(Player player) {
        return player.isOp() || player.hasPermission("premiumhunter") || granted.contains(player.getUniqueId());
    }
    ...
    public String describeStatus(OfflinePlayer target) {
        ...
        if (target.getPlayer() != null && target.getPlayer().hasPermission("premiumhunter")) return "§dPermission";
